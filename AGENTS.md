...
- **Adding a scenario**: copy `scenarios/_template/`, follow its comments, fill `cve-meta.json`. **Acceptance gate**: S01 and S05 must both be passing (test_gate.json shows `"passed":true`) before a new scenario is considered complete. S4 (Jinja2 sandbox escape) is now operational alongside S1 and S5.
