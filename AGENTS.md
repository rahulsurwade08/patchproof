...
- **Exploits and patch tests run in the TrueForge sandbox only — never on the host.** Service and PoC must run in the same sandbox instance (PoC relies on a shared `/tmp` marker file). The agentic pipeline always uses TrueForge + Daytona; `scripts/run_poc_local.sh` is the keyless alternative for human/CI-run verification only, satisfying the same contract via one network-isolated container (exit codes: 0 exploitable, 1 not affected, 3 service-start failure, 4 PoC timeout).
