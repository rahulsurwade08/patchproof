...
- **Qodo API key must not block the loop** — if the key/API is flaky, revert to the old method: pull findings via `gh api repos/:owner/:repo/pulls/<n>/comments` + GraphQL `reviewThreads` and run `/review` until clean, rather than waiting on `POST /rules/search`.
- **PR description needs 3-part compliance** — every PR body must state (1) work-item/requirement ID/link, (2) which README/docs section was reviewed/updated, and (3) test status. Qodo checks this via `test_e2e.py[16-18]` and similar.
