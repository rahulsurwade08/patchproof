...
- **Qodo clean is a hard gate — never rush a merge.** No PR may be merged with open Qodo findings (`Bugs >0` or `Rules >0` or any unresolved thread). If a PR is merged and Qodo later posts findings (post-merge review), the *next* PR is a fix-forward that addresses those findings first — no PR may be opened or merged until the fix-forward reaches `Bugs 0/Rules 0`, all threads resolved, and is merged. This was required to fix-forward PR #65's 2 post-merge findings (`docs/harness-loop-plan.md:45/49`) in PR #66.
