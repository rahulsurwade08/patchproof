...
- `harness/` — custom harness build (ADR-018): `harness/frontend/` (React + Vite on `@truefoundry/trueforge-ui`); `scripts/harness_setup.py` idempotently registers MCPs (`/api/v1/settings/mcp-servers`, PUT upsert by name), skills (`/api/v1/settings/skills`, PUT upsert), and upserts the patchproof-v2 agent manifest (GET `/api/v1/agents` → PUT `/api/v1/agents/{id}`); manifest uses `model.name`, name-only skill references, `mcp_servers` with per-MCP approval policies, and `config.sandbox`; startup: `python3 scripts/harness_setup.py` after `python3 agent/mcp/local_sandbox_server.py &` + `python3 agent/mcp/cve_feed_server.py &` + `npx @truefoundry/trueforge --port 8790 &`; `harness/tests/integration/test_agent_manifest.py` verifies the live manifest against spec; `harness/tests/` — test suite v2 (recreated per component as PRs land; see `docs/custom-harness-build-plan.md` §4)
