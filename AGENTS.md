...
- **Qodo clean is a hard gate — never rush a merge.** No PR may be merged with open Qodo findings (`Bugs >0` or `Rules >0` or any unresolved thread). If a PR merges and Qodo later posts findings (post-merge review), the *next* PR is a fix-forward that addresses those findings first — no new feature PR may be opened until the fix-forward reaches `Bugs 0/Rules 0`, all threads resolved, and is merged. Do not open or merge a new feature PR while fix-forward findings remain.
