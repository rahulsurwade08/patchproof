...
- **Qodo review loop must not be a hard blocker** — findings surface through Qodo's own review tools; if that path is flaky, fall back to whatever reading tools are available (e.g. the review threads you can see) and re-trigger review until clean. Keep acceptance **outcome-based and tool-agnostic** per the hard rule — never make a specific API/CLI sequence a mandatory part of the acceptance workflow.
