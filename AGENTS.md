...
- **caveman input/output + qodo skills are mandatory in every loop, every session, on every model.** The agent must use **caveman input and output** (terse, compressed communication) and load/use the **qodo skills** (`qodo-get-rules` before coding, `qodo-pr-resolver` when resolving findings) for **all** work — this loop, PR loops, and every future session — regardless of which model runs it. This is a hard rule, not a guideline; it applies even when a session is resumed, switched, or run under a different provider/model.
