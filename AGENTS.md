...
- **All testing goes through the harness (MCP), never on the host.** Every test — unit, integration, scenario, e2e — must run via `sandbox_build`/`sandbox_exec` through the local-sandbox MCP (`http://127.0.0.1:8081/mcp`) or the TrueForge harness API (`POST /api/v1/sessions/{id}/turns`). Direct `pytest`, `python`, or `docker run` on the host is forbidden. The only exception is `scripts/run_poc_local.sh` (CI/human path) which is itself a sandbox wrapper.
...
- **Exploits and patch tests never run on the host.** All execution goes through our `local-sandbox` MCP server (`agent/mcp/local_sandbox_server.py`, Python stdlib, Streamable HTTP at `127.0.0.1:8081/mcp`): `sandbox_build` (host-side image build, build-time network allowed, optional `dockerfile` name relative to the context — gen_context output (`Dockerfile.patchproof`) must be built with it; two-tier build per ADR-017: synthesized minimal first, escalation to the repo's own declared Dockerfile (`fallback_dockerfile` in patchproof-build-context.json) when its install fails, reported in the reproducer summary), `sandbox_exec`/`sandbox_write`/`sandbox_read`/`sandbox_pull`/`sandbox_stop` (offline `--network none` containers, one per session sharing `/tmp`; `image` is REQUIRED on every `sandbox_exec`/`sandbox_write`/`sandbox_pull` call). **`sandbox_pull`** is the reproducer's last call — it copies `/srv/verdict.json` to a host path (e.g. `data/output/<repo>/verdict.json`) so the canonical copy survives container teardown. **Start it before any harness run**: `python3 agent/mcp/local_sandbox_server.py &` (no dependencies). No cloud sandbox providers are used — TrueForge's built-in provider is paid and stays unconfigured (see ADR-008 for evaluated open-source alternatives); the agent manifest sets `config.sandbox.enabled: false` so the built-in `exec`/`shell` tool is not exposed. The agent's tool allowlist is in `docs/agent-sandbox-boundary.md` and `agent/prompts/system.md`; the user prompt template is in `agent/prompts/user_template.md`; the in-UI welcome message is in `agent/prompts/welcome.md`. `scripts/run_poc_local.sh` remains the no-harness human/CI path (exit codes: 0 exploitable, 1 not affected, 3 service-start failure, 4 PoC timeout). The dual-source `cve-feed` server (`agent/mcp/cve_feed_server.py`, CVE.org + OSV.dev) is also HTTP at `127.0.0.1:8091/mcp` (port via `CVE_FEED_PORT`); the Node stdio server was retired in PR #26.
...
- **Qodo clean is a hard gate — never rush a merge.** No PR may be merged with open Qodo findings (`Bugs >0` or `Rules >0` or any unresolved thread). If a PR is merged with open Qodo findings (whether visible before merge or posted afterward as a post-merge review), the *next* PR is a fix-forward that addresses those findings first — no *other* PR may be opened or merged until that fix-forward reaches `Bugs 0/Rules 0`, all threads resolved, and is merged. This was required to fix-forward PR #65's 2 post-merge findings (`docs/harness-loop-plan.md:45/49`) in PR #66.
- **qodo skills are mandatory in every session, on every model.** The agent must load/use the **qodo skills** where applicable — `qodo-get-rules` before any coding task, `qodo-pr-resolver` when resolving review findings. Applies to this loop, PR loops, and every future session regardless of which model runs it — even when a session is resumed, switched, or run under a different provider/model.
...
- **The "always pass image" lesson applies to callers, not just the server.** After PR #47 clamped `ensure_container` to recreate on image mismatch, the gate script's own PoC-session `sandbox_write`/health-check/PoC `sandbox_exec` calls still omitted `image` — so each one silently recreated the container with the default `python:3.11-slim`, wiping the just-started service (`FAIL: service failed to start in sandbox`). Audio/visual symptoms of a recreated container (empty `/srv`, background proc gone) point at a missing `image` param, not the server. Also: a `docker exec sh -c "svc ... &"` background job dies with its exec when the pipes close — start services with `nohup` + stdio redirects.
...
- **NEVER display secrets in session** — I violated this by reading `.env` and showing API keys (OpenRouter, GitHub) in chat output. The hard rule says "refer to keys by name only". When reading config files, use `grep -v "KEY\|TOKEN\|SECRET"` or similar to redact sensitive lines before displaying.
...
- **Frontend scope is decided by the maintainer, not the agent** — building custom UIs on `@truefoundry/trueforge-ui` consumes significant budget (theme tokens, slot overrides, proxied dev server, peer-dep overrides like `zustand:5.0.15` to satisfy `@assistant-ui/core`'s `useShallow` import). When the user redirects priorities to end-to-end testing, the frontend branch is parked and the work reverts to the stock TrueForge harness (`http://[::1]:8790`); do not re-engage custom-UI work without explicit go-ahead.
- **`sandbox_build` schema is `context_path` (absolute) + `tag`** — early callers passed `context` (a relative path), which the MCP rejects. Always pass the absolute host path and a unique image tag; `files` override exists for patched `requirements.lock` injection (ADR-013). Confirmed via the live `tools/list` schema dump.
- **`curl` is NOT in `python:3.11-slim`** — health-checks against a service started inside the sandbox must use `python3 -c "import urllib.request; ..."` or `wget` (if present), not `curl`. The reproducer/test-runner skill prompts must use a Python health-probe instead.
- **`/srv` is the image WORKDIR; `cve-meta.json` is NOT in the build context** — the scenario Dockerfile only copies the `app/` contents (Dockerfile, main.py, requirements.lock, test_main.py). The reproducer must `sandbox_write` `cve-meta.json` and `poc.py` from the host into `/srv/` before Phase 2. The first end-to-end run missed this and read `/srv/cve-meta.json` → "No such file or directory".
- **PoC URLs with single-quote injection must be `urllib.parse.urlencode`d** — s06 PoC's `/search?q=' OR '1'='1` raised `URL can't contain control characters. "/search?q=' OR '1'='1"` (found at least ' ') in the container's urllib. The fix is `urllib.parse.urlencode({"q": q})`. The PoC contract's <60s budget can absorb this; it's a real bug, not a logging artifact.
- **Qodo automated re-review is the merge signal** — never call `resolveReviewThread` after a fix push. Qodo posts its re-review 4–6 minutes after the push and the new header counts (Bugs/Rules) and thread status are the only authoritative signal. Manually resolving threads before Qodo's re-review has been observed to leave threads showing open even when the finding is fixed.
- **Service start inside sandbox needs `nohup` + stdio redirects, not `&`** — `docker exec sh -c "uvicorn ... &"` background processes die with the exec shell. Reproducer must `nohup uvicorn ... > /tmp/uv.log 2>&1 &` inside a `python3 -c` wrapper or a one-shot `bash -lc` so the server outlives the `sandbox_exec` call that started it (matches the gate-script pattern from PR #64).
- **TrueForge's stock UI is the dev/test surface for now** — once the frontend branch is parked, `http://[::1]:8790` (the TrueForge standalone UI) is the visible harness. A custom PatchProof UI remains a next-release decision owned by the maintainer. Do not burn cycles re-skinning the SDK without a directive.
...
- `agent/` — Python runtime: subagent prompts (`agent/prompts/`: orchestrator, analyzer, reproducer, judge, patcher, verifier, test-runner), the Python reachability analyzer (`agent/analyzer/*.py`), dual-source CVE feed MCP server (`agent/mcp/cve_feed_server.py`), local Docker sandbox MCP server (`agent/mcp/local_sandbox_server.py`, Streamable HTTP on `127.0.0.1:8081/mcp`, with `gen_build_context`, `sandbox_build`, `sandbox_exec`, `sandbox_write`, `sandbox_read`, `sandbox_pull`, `sandbox_stop`), and harness skills (`agent/skills/`: analyzer, orchestrator, reproducer, judge, patcher, verifier, test-runner; `cve-triage` retired). Also: `agent/prompts/system.md` (agent boundary rules in context window), `agent/prompts/user_template.md` (user prompt template), `agent/prompts/welcome.md` (in-UI first-contact message).)
- `harness/` — custom harness build (ADR-018): `harness/frontend/` (React + Vite on `@truefoundry/trueforge-ui`); `scripts/harness_setup.py` idempotently registers MCPs (`/api/v1/settings/mcp-servers`, PUT upsert by name), skills (`/api/v1/settings/skills`, PUT upsert), and upserts the patchproof-v2 agent manifest (GET `/api/v1/agents` → PUT `/api/v1/agents/{id}`); manifest uses `model.name`, name-only skill references, `mcp_servers` with per-MCP approval policies, and `config.sandbox` (set `enabled: false` to hide TrueForge's paid built-in provider); startup: `python3 scripts/harness_setup.py` after `python3 agent/mcp/local_sandbox_server.py &` + `python3 agent/mcp/cve_feed_server.py &` + `npx @truefoundry/trueforge --port 8790 &`; `harness/tests/integration/test_agent_manifest.py` verifies the live manifest against spec; `harness/tests/` — test suite v2 (recreated per component as PRs land; see `docs/custom-harness-build-plan.md` §4)
- `scripts/agent/run_session.py` — drives a TrueForge session end-to-end: renders `agent/prompts/user_template.md` from a JSON/KEY=VALUE fields file, auto-approves all tool calls, streams model text, writes per-turn telemetry to `data/output/.patchproof/runs/<sid>.jsonl`. Usage: `python3 scripts/agent/run_session.py <session_id> [fields.json]`.
