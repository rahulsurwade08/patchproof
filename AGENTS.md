...
- **Exploits and patch tests never run on the host.** The agentic sandbox is our own `local-sandbox` MCP server (`agent/mcp/local-sandbox-server/index.mjs`, Streamable HTTP at `127.0.0.1:8081/mcp`): disposable Docker containers with `--network none`, one per session so service and PoC share `/tmp`. TrueForge's built-in Daytona provider stays disabled (paid; incompatible with open-source). `scripts/run_poc_local.sh` remains the no-harness human/CI path (exit codes: 0 exploitable, 1 not affected, 3 service-start failure, 4 PoC timeout).
- **Start the sandbox server before any harness run**: `node agent/mcp/local-sandbox-server/index.mjs &` (no dependencies).
...
- **TrueForge has no config file** (verified against v0.1.4 docs): models/connectors/skills/sandbox are configured via Settings — see `docs/trueforge-setup.md`. Built-in sandbox provider is **Daytona only** (paid) and stays disabled; we use the local-sandbox MCP server instead. MCP servers are remote-URL only; stdio servers need an HTTP wrapper or must be bypassed via `data/inbox/` injection.
