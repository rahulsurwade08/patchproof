...
- [#8 — LLM-as-a-judge subagent + credential guidance](https://github.com/rahulsurwade08/patchproof/pull/8): Qodo raised 3 findings — judge depended on cve-feed tools that can't be registered yet, a judge-triggered retry left a stale assessment, and the initial gh-token guidance clashed with the no-secrets-in-session compliance rule. Fixed in 5809971 and 1391e0f: explicit degraded-mode contract (`range_check: "skipped"`), re-judging of retried verdicts with routing on the latest machine verdict, and OAuth-first GitHub credentials (no static token; PAT documented as fallback only).
