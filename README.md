...
- [#8 — LLM-as-a-judge subagent + gh-token-first credentials](https://github.com/rahulsurwade08/patchproof/pull/8): Qodo raised 3 findings — judge depended on cve-feed tools that can't be registered yet, a judge-triggered retry left a stale assessment, and the gh-token guidance clashed with the no-secrets-in-session compliance rule. Fixed in 5809971: explicit degraded-mode contract (`range_check: "skipped"`), re-judging of retried verdicts with routing on the latest machine verdict, and display-safe credential guidance (`gh auth token` redirected straight into `.env` by the human, never shown or handled by agents).
