...
- [#45 — docs: mandate caveman i/o + qodo skills as hard rule](https://github.com/rahulsurwade08/patchproof/pull/45): Qodo raised 2 findings across 2 cycles, all resolved (Bugs 0/Rules 0). Added AGENTS.md hard rule: caveman input/output + qodo skills (`qodo-get-rules` before coding, `qodo-pr-resolver` when resolving) mandatory in every session/model. Scoped qodo skills to "where applicable" and superseded ADR-015 communication-style deferral (compression-technique deferral + full-prose judge/approval unchanged) via `docs/decisions.md`.
