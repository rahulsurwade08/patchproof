...
- [#39 — fix: OSV _select_dep ecosystem, ranges, lookup](https://github.com/rahulsurwade08/patchproof/pull/39): Qodo raised 2 findings across 2 cycles, all resolved (Bugs 0/Rules 0). Fixed `_select_dep` ecosystem filtering (unsupported non-empty → no match, empty → unrestricted), version-or-range OR logic with `""` all-versions sentinel, and Python lookup via both PEP 503 and npm keys. Updated `plan.md` Honesty rules with dependency scope. Verified 99 analyzer tests pass.
- [#40 — docs: decadal audit 40 — stale ref polish](https://github.com/rahulsurwade08/patchproof/pull/40): Qodo found no issues (Bugs 0/Rules 0). Decadal audit per AGENTS.md: scanned stale refs, docs vs code, secrets, placeholders. Fixed `docs/architecture.md` local-sandbox row phrasing (`migrating from` → `migrated from in PR #28`).
- [#41 — docs: mark cut-order complete](https://github.com/rahulsurwade08/patchproof/pull/41): Qodo found no issues (Bugs 0/Rules 0). Plan cut-order all four items completed 2026-08-28.
- [#42 — test: e2e smoke for analyzer reach](https://github.com/rahulsurwade08/patchproof/pull/42): Qodo raised 2 findings across 2 cycles, all resolved (Bugs 0/Rules 0). Added `agent/analyzer/tests/test_e2e.py` smoke covering `reach.reach()` → `reachability.json` for pyyaml `yaml.load()` case, asserting `NOT_REACHABLE` with direct sites. Updated PR description with traceability (plan.md §5, ADR-010) and docs review. Verified 1 passed.
- [#43 — docs: Qodo evidence for #42](https://github.com/rahulsurwade08/patchproof/pull/43): Qodo found no issues (Bugs 0/Rules 0). Backfilled the README Qodo Code Review Evidence entry for #42 (e2e smoke).
- [#44 — docs: capture lessons from opencode-mem, qodo loop, dash gate](https://github.com/rahulsurwade08/patchproof/pull/44): Qodo raised 3 findings across 2 cycles, all resolved (Bugs 0/Rules 0). Added 6 AGENTS.md "Lessons learned": opencode-mem explicit config+model, Qodo-clean-gate-mandatory/tool-path-optional (tool-agnostic wording), PR 3-part description baseline, smoke-test specific verdict, dashboard-gate stdin-awareness, OSV `_select_dep` ecosystem/OR semantics. Docs-only.
- [#45 — docs: mandate qodo skills as hard rule](https://github.com/rahulsurwade08/patchproof/pull/45): Qodo raised 2 findings across 2 cycles, all resolved (Bugs 0/Rules 0). Added AGENTS.md hard rule: qodo skills (`qodo-get-rules` before coding, `qodo-pr-resolver` when resolving) mandatory in every session/model. Scoped qodo skills to "where applicable" and superseded ADR-015 deferral (compression-technique deferral + full-prose judge/approval unchanged) via `docs/decisions.md`. Note: a terse-communication style briefly mandated alongside this rule was reverted in #49.
- [#46 — docs: README Qodo evidence for #43 #44 #45](https://github.com/rahulsurwade08/patchproof/pull/46): Qodo found no issues (Bugs 0/Rules 0). Backfilled README Qodo Code Review Evidence for #43, #44, #45.
- [#47 — fix(sandbox): stop container recreation on every exec](https://github.com/rahulsurwade08/patchproof/pull/47): Qodo raised 1 security finding across 2 cycles, all resolved (Bugs 0/Rules 0). Found during full harness e2e testing: `ensure_container` recreated the container on every `sandbox_exec`/`sandbox_write` (wiping session state) because Docker reported `--network none` as a literal `none` key that the empty-map-only match rejected; and the inspect template concatenated network keys with no delimiter (keys `no`,`ne` → `none`). Fix: emit one network key per line (`{{println $k}}`), parse exact keys; a `--network none` container matches only an empty set or the literal `none` key. Regression tests (`test_ensure_container_reuses_literal_none_network_key`, `test_ensure_container_reject_concat_named_networks`) plus 2 existing mcp tests. Live-verified: state persists across back-to-back execs and full s01 build→write→exec→verdict→read→stop e2e.
