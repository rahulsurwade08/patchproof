...
**What the SDK gives** ([Chat UI](https://trueforge.dev/chat-ui), [UI SDK](https://trueforge.dev/ui-sdk), live at `https://ui.trueforge.dev`): `TrueForgeUI` with `server={{type:"trueforge", baseUrl}}` and `agentConfig={{mode:"SingleAgent", name:"patchproof-v2"}}`, `Chat` (streaming `Turn` `model.message`/`tool.call`/`sandbox_artifacts`, approvals, file attachments as session-turn input — advisory JSON attached in the UI becomes turn input; `data/inbox/*.json` remains the host-side advisory drop dir, optionally mirrored from turn attachments via `harness/backend/server/patchproof-bridge.ts`), `Themes` (`truefoundry` + custom), `Layouts` (full-screen sidebar vs embedded drawer/widget).
