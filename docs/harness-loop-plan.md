...
| **MCP cve-feed** | Settings → Connectors → Add MCP Server | `agent/mcp/cve_feed_server.py` (Python stdlib, Streamable HTTP at `http://127.0.0.1:8091/mcp`) — transport done in PR #71, remote-URL registration done in PR #72; `data/inbox/*.json` (`demo:true` → `demo-bypass`, fail-closed ADR-010) remains the unit-test fallback | — |
