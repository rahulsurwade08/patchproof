...
harness/
  tests/
    ...
    integration/
      ...
      test_agent_manifest.py    # patchproof-v2 manifest: model, mcp_servers, skills[7], sandbox.enabled:true, approvals ([local-sandbox:@all, cve-feed:@write|@destructive, github:default])
    e2e/
...
| 5 `feat/approval-gate` | manifest `require_approval_for_tools` per MCP server (API) — done in PR #73: `local-sandbox=["@all"]` (every sandbox tool call requires human approval before the irreversible exploit-execution step); `cve-feed=["@write","@destructive"]` (read-only data with `readOnlyHint: true`); `github` uses default (PRs via `github` MCP use `@write` gating, patcher stops before merge/staging) | harness turn pauses before any `local-sandbox` tool call; PRs are opened via github MCP (not sandbox); patcher stops before merge/staging |
