...
| 3 attach skills + MCPs | `scripts/harness_setup.py` (register skills/MCPs/agent) + updated docs — done in PR #72 | `GET /api/v1/skills` lists 7; `GET /api/v1/settings/mcp-servers` lists 2 (local-sandbox + cve-feed); the github connector is added separately via the TrueForge catalog UI per `docs/trueforge-setup.md` §3 and referenced in the agent manifest as `ATTACHED_NOT_REGISTERED`; agent manifest matches spec |
