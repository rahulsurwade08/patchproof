...
| 5 `feat/approval-gate` | manifest `require_approval_for_tools` per MCP server (API) — done in PR #73: `local-sandbox=["@all"]` (every sandbox tool call requires human approval before the irreversible exploit-execution step); `cve-feed=["@write","@destructive"]` (read-only data, write/destructive gated); `github` uses default | harness turn shows approval pause before any `local-sandbox` tool call; PR/staging deploys are subagent-driven inside the sandbox (irreversible steps) |
