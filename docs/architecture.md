...
| cve-feed | `agent/mcp/cve_feed_server.py` (Python stdlib, Streamable HTTP at `127.0.0.1:8091/mcp`) | Dual-source legitimacy: CVE.org canonical records + OSV.dev package matching (`cve_get_cve`, `osv_query_package`, `osv_get_vuln`, `cve_cross_check`) — registered as remote-URL MCP in TrueForge; advisories also accepted via `data/inbox/` |
...
| Test suite v2 | `harness/tests/` (planned, recreated per component as PRs land) | Unit (cve-feed HTTP transport, sandbox regressions), integration (skills/MCP registration, frontend build, agent manifest), e2e (session turn → verdict, approval pause) — see `docs/custom-harness-build-plan.md` §4 |
