...
| Harness frontend | `harness/frontend/` (React + Vite on `@truefoundry/trueforge-ui`) | Custom chat UI against the stock TrueForge server (`http://[::1]:8790`), `SingleAgent patchproof-v2`; renders `sandbox_artifacts` (reachability/verdict/assessment) and the approval checkpoint (ADR-018) |
| Test suite v2 | `harness/tests/` | Recreated per attached component: unit (cve-feed wrapper, sandbox regressions), integration (skills/MCP registration, frontend build, agent manifest), e2e (session turn → verdict, approval pause) — see `docs/custom-harness-build-plan.md` §4 |
...
| Skills | `analyzer` (first stage) + `orchestrator`/`reproducer`/`judge`/`patcher`/`verifier`/`test-runner`; `cve-triage` retired; registered via Settings → Skills (git, pinned SHA); requires agent `config.sandbox.enabled: true` per official docs (ADR-018) |
