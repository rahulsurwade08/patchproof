...
| Time overrun | Cut order: analyzer (Python) + build-context gen → MCP migration → osv wiring polish → dashboard. Approval gate never cut — **cut-order items 1–5 complete 2026-08-28** (analyzer + gen_context, Python MCP servers, OSV _select_dep polish, dashboard hermetic suite + gate). **ADR-018 harness build mostly complete 2026-08-29** (frontend scaffold PR #68, cve-feed HTTP PR #71, skills+MCP attach PR #72, approval gate PR #73, integration tests PR #74). **Remaining** (per ADR-018 test-suite-v2, see `docs/custom-harness-build-plan.md` §4): (a) harness-driven end-to-end scenario test via `POST /api/v1/sessions/{id}/turns` to drive `sandbox_build` + `sandbox_exec` and assert `verdict.json` + `assessment.json` (placeholder in `harness/tests/integration/test_scenarios.py`); (b) approval-gate e2e test asserting the harness pauses with `awaiting_approval` before any `sandbox_build`/`sandbox_exec`; (c) plan-docs sync to the next round.
