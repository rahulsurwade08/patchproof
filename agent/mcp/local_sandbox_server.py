...
Tools:
  ...
  gen_build_context -- synthesize Dockerfile.checkexploit for a target repo
...
SERVER_INFO = {"name": "checkexploit-local-sandbox", "version": "0.1.0"}
...
LABELS = ["--label", "checkexploit-sbx=1"]
...
def container_name(session):
    ...
    return f"checkexploit-sbx-{digest}"
...
def cleanup_all_containers():
    ...
    listed = docker(["ps", "-aq", "--filter", "label=checkexploit-sbx=1"])
...
TOOLS = [
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "dockerfile": {"type": "string", "description": "Dockerfile name RELATIVE to context (default 'Dockerfile'), e.g. 'Dockerfile.checkexploit'"},
            },
    ...
    },
    {
        ...
        "description": "Synthesize Dockerfile.checkexploit for an arbitrary repo. Returns build_context + tag. Call BEFORE sandbox_build.",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "force": {"type": "boolean", "description": "overwrite an existing Dockerfile.checkexploit"},
            },
            "required": ["repo_path"],
        },
    },
...
def _clone_repo(args):
    ...
    clone_root = tempfile.mkdtemp(prefix="checkexploit-clone-")
...
def tool_call(name, args=None):
    ...
    if name == "sandbox_build":
        ...
        try:
            if files:
                tmp_dir = tempfile.mkdtemp(prefix="checkexploit-ctx-")
    ...
    if name == "gen_build_context":
        # Synthesize Dockerfile.checkexploit for an arbitrary target repo. This
        ...
        if not out_dir:
            out_dir = tempfile.mkdtemp(prefix="checkexploit-ctx-")
