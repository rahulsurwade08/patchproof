...
def test_e2e_s01_reach(tmp_path):
    ...
    adv = {"cve_id": "CVE-2020-14343", "description": "yaml.load() vulnerable in pyyaml", "packages": [{"name": "pyyaml", "ranges": ["<= 5.3.1"], "versions": [], "ecosystem": "pypi"}]}
    out = tmp_path / "out"
    rec = reach.reach(str(repo), adv, str(out))
    assert rec["verdict"] == "NOT_REACHABLE"
    assert rec["call_sites"]["direct"]
    assert rec["call_sites_scanned"][0]["input_source"] == "NOT_REACHABLE"
    assert (out / "reachability.json").exists()
    data = json.loads((out / "reachability.json").read_text())
    assert data["cve_id"] == "CVE-2020-14343"
    assert data["verdict"] == "NOT_REACHABLE"
