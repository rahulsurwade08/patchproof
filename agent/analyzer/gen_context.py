...
_BASE_ALLOWLIST_RE = re.compile(r"^(python|node)(:[\w.\-]+)?$")
    ...
    for rel in files:
        try:
            with open(os.path.join(repo_path, rel), encoding="utf-8",
                ...
                for line in (head,) + tuple(fh):
                    m = _FROM_RE.match(line)
                    if not m:
                        continue
                    base = m.group(1).split(" AS ")[0].split(" as ")[0].strip()
                    if "${" in base or not _BASE_ALLOWLIST_RE.match(base):
                        continue  # unresolvable or untrusted — skip file
