...
def _official_base_name(img):
    """True iff `img` refers to a STRICT unqualified official python/node
    image (python, node, library/python, library/node, docker.io/library/...).
    Anything else — including foreign/registry/organization-qualified names
    whose last segment happens to be python/node — is rejected. Names with
    variable components or digests are rejected."""
    name = img.split(":")[0].split("@")[0]  # strip tag/digest to the repo path
    segs = [x for x in name.split("/") if x]
    if not segs:
        return False
    if segs[0] == "docker.io":
        segs = segs[1:]
