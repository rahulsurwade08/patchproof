...
def _first_declared_dockerfile(repo_path):
    ...
    runtime FROM base is an ALLOWLISTED official `python`/`node` image
    qualifies (ADR-016: repo content stays inert data; ARB-017 tier-2 keeps
    the strict official-base restriction rather than executing arbitrary
    repo-controlled base images)."""
    preferred = ["Dockerfile", "Dockerfile.app", "Dockerfile.web",
                 "Dockerfile.prod", "Dockerfile.api", "Dockerfile.server"]
    files = []
    for root, dirs, names in os.walk(repo_path):
        ...
        for fname in names:
            if fname == "Dockerfile" or fname.startswith("Dockerfile."):
                ...
                for line in lines:
                    ...
                    if "${" in img:
                        continue
                    name = img.split("/")[-1].split(":", 1)[0].split("@", 1)[0]
                    allowlisted = name in ("python", "node")
                if allowlisted:
