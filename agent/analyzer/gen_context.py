...
#  Dockerfile while capping memory against a hostile oversized candidate
_TAG_RE = re.compile(r"^[A-Za-z0-9_][A-Za-z0-9._-]{0,127}$")


def _official_base_name(img, lang):
    """True iff `img` is a STRICT official base matching the project's
    runtime language `lang` ("py" -> python only, "js" -> node only).
    Allowed: python/node, library/python|node, docker.io/library/... with an
    optional VALID `:<tag>`. Rejected: digest-qualified references
    (`@sha256:...`), empty/malformed tags (`python:`, `python:bad/tag`,
    `node:bad!`), and any foreign/registry/organization-qualified name whose
    last segment merely equals python/node (ADR-016/ADR-017: the tier-2
    fallback keeps the strict official-base restriction AND must run the
    recorded startup command, so the base language must match the entry)."""
    if "@" in img:
    ...
    if len(segs) != 1:
        return False
    if lang == "py":
        return segs[0] == "python"
    if lang == "js":
        return segs[0] == "node"
    return False


def _first_declared_dockerfile(repo_path, lang):
    ...
    runtime (last) FROM base is a STRICT allowlisted official image that
    MATCHES the project language qualifies (ADR-016: repo content stays
    inert data; ADR-017 tier-2 keeps the strict official-base restriction
    rather than executing arbitrary repo-controlled base images). Foreign/
    registry-qualified pseudo-official names and variable FROM are rejected.
    Reads are bounded by _MAX_DOCKERFILE_LINES to cap memory against a
    hostile oversized candidate."""
    preferred = ["Dockerfile", "Dockerfile.app", "Dockerfile.web",
                 "Dockerfile.prod", "Dockerfile.api", "Dockerfile.server"]
    files = []
    for root, dirs, names in os.walk(repo_path):
            ...
            if fname == "Dockerfile" or fname.startswith("Dockerfile."):
                ...
                try:
                    ...
                    with open(path, encoding="utf-8",
                        ...
                        lines = fh.readlines(_MAX_DOCKERFILE_LINES)
                except OSError:
                ...
                for line in lines:
                    ...
                    if "${" in img or "$" in img:
                        ...
                        continue
                    allowlisted = _official_base_name(img, lang)
                if allowlisted:
...
def generate(repo_path, out_dir=None, force=False):
    ...
    fallback = _first_declared_dockerfile(repo_path, "py" if is_py else "js")
