...
def _detect_base_image(repo_path, entry_lang):
    """Return (base_image, dockerfile_rel) for the repo's declared runtime.

    The declared Dockerfile is used VERBATIM as the sandbox build definition
    — its RUN steps (apk/apt build tools etc.) are exactly what makes the
    repo's pinned dependencies installable, which a synthesized minimal
    Dockerfile cannot guess. Scans prioritized Dockerfile* files.

    Returns (base, rel) where rel names the FIRST file whose FROM is an
    allowlisted official python/node image (well-formed tag or 64-hex
    digest) matching the entry language. Anything else — parametrized
    `FROM ${VAR}`, foreign images, malformed refs, wrong language — is
    skipped and the scan CONTINUES; (None, None) only when no file yields
    an acceptable FROM (the caller then synthesizes or fails explicitly).
    Our own generated artifacts are marker-skipped. Legacy apps (e.g.
    dvpwa) pin python:alpine3.8-era bases whose build tooling their own
    Dockerfile declares; a synthesized minimal Dockerfile would fail.
    """
...
def generate(repo_path, out_dir=None, force=False):
    ...
    base, declared_df = _detect_base_image(repo_path, "py" if is_py else "js")

    if declared_df:
        # The repo declares a well-formed, allowlisted runtime Dockerfile:
        # use it VERBATIM — its RUN steps (apk/apt build tooling etc.) are
        # exactly what makes the pinned dependencies installable, which a
        # synthesized minimal Dockerfile cannot guess.
        with open(os.path.join(repo_path, declared_df), encoding="utf-8") as fh:
            declared_content = fh.read()
        _write_file(os.path.join(out_dir, "Dockerfile"), declared_content, force)
        _write_file(os.path.join(out_dir, ".dockerignore"),
                    "\n".join((".git", ".env", ".env.*", "*.pem", "*.key",
                                "*.p12", "*.pfx", "id_rsa*", ".aws", ".ssh",
                                ".npmrc", ".pypirc", "credentials*",
                                "*.credentials", "venv", ".venv",
    ...
    if _repo_declares_unusable_dockerfile(repo_path):
        raise ValueError(
            f"repo declares Dockerfile* files but none has an allowlisted "
            f"official python/node base (repo content is untrusted, "
            f"ADR-016); supply a usable build context or base explicitly")
